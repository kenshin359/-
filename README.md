...
このリポジトリには、株式会社リベティ（Libetee）向けの2つのツールが入っています。
...
> **会社分離ポリシー**：このリポジトリは**株式会社リベティ（Libetee）専用**です。別会社（例：O2ジム）のツール・広告アカウント・kintone・トークンは本リポジトリに混在させません。O2ジムの広告売上KPI自動連携は**O2専用リポジトリ**に分離しています。
