...
/**
 * スタッフ用LINEグループのID。
 * 優先順: 環境変数 LINE_STAFF_GROUP_ID > 公式アカウントをグループに招待したときに自動登録した値（Setting テーブル）。
 */
export const STAFF_GROUP_KEY = 'line.staffGroupId';

export async function getStaffGroupId(): Promise<string> {
  const env = (process.env.LINE_STAFF_GROUP_ID || '').trim();
  if (env) return env;
  try {
