...
/**
 * グループ・複数人トークでのイベント処理。
 * - join: 通知先として自動登録（環境変数 LINE_STAFF_GROUP_ID があればそちらが優先）
 * - leave: 登録解除
...
export async function handleGroupEvent(ev: GroupEvent): Promise<void> {
  ...
  if (ev.type === 'join') {
    ...
    const envFixed = Boolean((process.env.LINE_STAFF_GROUP_ID || '').trim());
  ...
  if (cmd.kind === 'register') {
    if ((process.env.LINE_STAFF_GROUP_ID || '').trim()) {
      await replyText(ev.replyToken, '通知先は環境変数 LINE_STAFF_GROUP_ID で固定されています。変更する場合は Vercel の設定を変えてください。');
      return;
    }
