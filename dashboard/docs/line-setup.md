...
## 注意: Vercel Hobby プランの制限
Hobby プランでは cron は1日1回までのため、`vercel.json` の定期処理は毎日 10:00 JST（`0 1 * * *`）に設定しています。追いかけリマインドと定時投稿はこの時刻にまとめて実行されます。10分ごとに戻すには Pro プランへ移行してから `*/10 * * * *` に変更してください。
