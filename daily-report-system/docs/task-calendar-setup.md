...
## 3-B. タスク報告部屋（スタッフが投稿→自動で登録・完了・退勤管理）

スタッフが専用ルームに決まった書式で投稿すると、ボットが回収して
タスクを登録・更新し、本人宛てに確認を返信します。カレンダーにも反映されます。

**① 報告部屋を作る**
```
npm run task:create-room       # 【タスク報告】を作成 → room_id を .env の CHATWORK_REPORT_ROOM に
```
...
```
```
退勤            ← 本日の残タスクを自動集計して本人に返信
```
- 「登録」の次の行に「・タスク名（期限）」を並べるだけ。期限は `8/15` や `2026-08-15`。
- 「完了／着手」はタスク名の一部でも一致します（本人のタスクのみ対象）。
- コマンド以外の通常メッセージは無視します。

**③ 回収と反映（定期実行）**
```
npm run task:collect           # 新規メッセージを処理＆本人へ確認返信（test中は送らずプレビュー）
npm run task:chatwork:sync     # 報告されたタスクをカレンダーへ反映
```

- 状態は `state/report-tasks.json` に保存（未完了・完了・退勤時の残タスクを管理）。
- cron 例：`*/10 * * * * … npm run task:collect` と `0 8 * * * … npm run task:chatwork:sync`。
