...
//  楽天の売上を、自動でキントーンに入れる
// ------------------------------------------------------------
//  CSVのダウンロードは要りません。RMSの公式APIから直接取ってきます。
//
//  実行:
//    npm run rakuten:import                      … 昨日ぶん
//    npm run rakuten:import -- --date=2026-07-31 … 指定した1日
//    npm run rakuten:import -- --from=2026-07-01 --to=2026-07-31
//    npm run rakuten:import -- --dry-run         … 書き込まず内容だけ表示
//
//  ★何度実行しても二重になりません。
//    その日の「楽天」ぶんだけを入れ替えます（Amazon・自社サイトは残ります）。
// ============================================================
import { fetchOrders, ordersToRows } from '../lib/rakutenPay.js';
...
async function main() {
  ...
  console.log(`楽天 から ${from} 〜 ${to} の注文を取ってきます …`);
  console.log(`SKU対応表: ${skuMap.size}件 登録済み`);

  const orders = await fetchOrders(from, to);
  const { rows, skipped } = ordersToRows(orders, { channel: CHANNEL });
  ...
  console.log(`売上 ${yen(total)} ／ ${qty}個 ／ 注文 ${orderCount}件`);
...
if (process.argv[1] && process.argv[1].endsWith('rakutenImport.js')) {
  main().catch((e) => {
