...
import { savePlan, loadPlan, carryOver, markStatus, analyze, planPath } from '../lib/secretary/store.js';
...
async function cmdPlan() {
  ...
  const plan = buildDailyPlan(text, { date, carry, stats: stats.days ? stats : null });
...
function cmdMark(status) {
  ...
  for (const t of res.tasks) {
    ...
    console.log(`${status === 'done' ? '✅' : status === 'dropped' ? '🗑' : '↩️'} ${t.title} ${actual}`);
  }
}
...
function cmdStatus() {
  ...
  const live = (plan.tasks ?? []).filter((t) => !t.deferred);
