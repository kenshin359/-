// 「SNS/LP週次報告（新）」アプリを自動で作成する。
//   1. アプリの器を作成（プレビュー環境）
//   2. フィールドを追加
//   3. 本番へデプロイ
// ※ アプリ作成はパスワード認証が必須（KINTONE_USER + KINTONE_PASSWORD）。
//   実行: npm run create-weekly-app
import { kintone, qs } from './client.js';
import { WEEKLY_FIELDS } from './weeklySchema.js';

const NAME = process.env.KINTONE_WEEKLY_APP_NAME || 'SNS/LP週次報告（新）';
...
async function main() {
  ...
  await kintone('POST', '/k/v1/preview/app/form/fields.json', { app, properties: WEEKLY_FIELDS });
  console.log(`  ② フィールド ${Object.keys(WEEKLY_FIELDS).length} 個を追加`);
  ...
  console.log('\n完了 ✅  新アプリが本番に反映されました。');
  console.log(`   .env に次を設定してください →  KINTONE_WEEKLY_APP_ID=${app}`);
}
