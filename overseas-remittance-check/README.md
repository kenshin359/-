...
openpyxlは数式を文字列として書き込むだけで、計算結果を持たせない。
つまり「数式が入っている」ことと「その数式が正しく動く」ことは別物になる。
とくに照合列は、常に「一致」と返すだけの数式でも見た目は正常に見えてしまう。

```bash
python3 scripts/verify_workbook.py 送金チェック_OMT20260901100481.xlsx
```

全数式を実際に計算し、エラーがないことと、照合列がすべて一致することを確認する。
`tests/test_workbook.py` はこれに加えて、セルの値を書き換えたら判定が反転すること
（＝照合表が実際に検査をしていること）まで確かめる。

生成時に `fullCalcOnLoad` を立てているので、Excelで開いた時点で再計算される。

### 環境上の注意

- **ファイル名**: `LANG` が未設定（POSIXロケール）の環境では、日本語のファイル名を
  外部コマンドに渡した時点で `?` に化ける。外部ツールにかけるときは一度ASCII名に
  コピーする。`build_check_sheet.py` は非ASCII名のときに警告を出す。
- **LibreOffice**: 環境によっては `soffice --headless` が起動段階でハングし、
  3セルだけのファイルでも応答が返らないことがある（CPUをほとんど使わずに待ち続ける）。
  その場合は上記の `verify_workbook.py`（純Python、外部プロセス不要）を使う。
