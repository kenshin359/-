...
精度を上げるため、返信の③④は2段階で作ります（`ANTHROPIC_API_KEY` 設定時）。

1. **1パス目**：商品名・種別を渡して下書きを生成（無い機能を書かせない／不満の誤読を防ぐ指示付き）
2. **2パス目（合議レビュー）**：**優秀なCS3人＋管理職＋役員**の5役になりきって下書きを点検・添削
   （誤読・商品ミスマッチ・語尾重複・できない約束・囲み引用符をチェック）。設定は `config/council.json`。

さらに機械的な安全ネット（`config/quality-check.json`）で、怪しい返信は 🔴 人の確認へ回します。
`council.enabled=false` にすると2パス目を止めて AI 呼び出しを1回に節約できます。
